...
> **Avvertenza:** I valori sotto riportati sono stime analitiche conservative basate sulle proprietà dei materiali e sulla geometria. NON sono basati su test di laboratorio. Non utilizzare questi valori per l'ingegneria strutturale senza verifica indipendente. Le prestazioni effettive dipendono dalla qualità dei materiali, dalla lavorazione e dai dettagli delle connessioni. Vedi [Test Necessari](#test-necessari) qui sotto.
...
Stima conservativa basata sulla resistenza al taglio della malta (τ ≈ 0,3 MPa per malta 1:4 cemento:sabbia) sulla sezione trasversale del pannello:

- Area di taglio: ~85 mm × 1.000 mm = 85.000 mm²
- Capacità di taglio conservativa: 85.000 × 0,3 = ~25 kN per pannello
...
- Sezione trasversale: 1.200 mm²
- Resistenza a trazione conservativa: 80 MPa (usando ~50% del valore pubblicato di 150–200 MPa)
- Capacità di trazione della diagonale: 1.200 × 80 = ~96 kN
- Componente orizzontale (cos 69°): ~96 × 0,36 = ~34 kN per diagonale
- Due diagonali (una per lato, una in trazione): ~34 kN
- Le legature in filo e l'incastro della malta aggiungono ulteriore resistenza
- Con fattore di sicurezza 3: **~12–15 kN carico di taglio orizzontale di esercizio per pannello**

**Per contesto:** Una sezione di parete larga 3 m (3 pannelli) fornisce ~36–45 kN di resistenza al taglio orizzontale. Un edificio residenziale monopiano in una zona sismica moderata (PGA 0,2g) con 30 kN di massa sismica sopra la parete necessita di circa 6 kN di resistenza al taglio alla base per metro lineare di parete. Tre pannelli per metro forniscono 4–5 volte questo requisito.

Questi sono numeri molto conservativi. La matrice di malta, il reticolo di legature in filo e il profilo )( contribuiscono tutti con resistenza aggiuntiva non contabilizzata sopra.
...
| Capacità stimata di pressione uniforme | ~0,7 kPa |
| Con fattore di sicurezza 2 | **~0,35 kPa carico di esercizio** |

**Per contesto:** 0,35 kPa ≈ velocità del vento ~25 m/s (90 km/h). Per zone a vento più forte, il telaio in acciaio (non il pannello) porta il carico del vento attraverso le connessioni trave-pilastro. Il pannello deve solo resistere alla pressione locale tra i propri elementi del telaio.
...
|------------|----------------------|-------------|-------------|
| Telaio in acciaio | Corrosione | Zincatura a caldo (85 μm+) | 80–100 anni (non costiero) |
