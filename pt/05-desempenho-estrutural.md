...
1. **Conversão cisalhamento → tração:** Quando a estrutura da edificação se deforma lateralmente, uma diagonal entra em tração enquanto a oposta entra em compressão. A diagonal de bambu em tração resiste à força de deformação eficientemente — o bambu tem excelente resistência à tração (150–200 MPa para Guadua angustifolia).
2. **Pré-tensão:** As diagonais são instaladas com leve pré-tensão, eliminando folga e garantindo engajamento imediato sob carga.
3. **Grade de amarrações de arame:** Amarrações de arame galvanizado em cada cruzamento diagonal-vertical (~16–20 amarrações no total) distribuem a força diagonal por toda a face do painel, prevenindo falha localizada.
...
1. Primeiro: a argamassa fissura (aviso visível)
...
Estimativa conservadora baseada na resistência ao cisalhamento da argamassa (τ ≈ 0,3 MPa para argamassa de cimento:areia 1:4) na seção transversal do painel:

- Área de cisalhamento: ~85 mm × 1.000 mm = 85.000 mm²
- Capacidade conservadora de cisalhamento: 85.000 × 0,3 = ~25 kN por painel
...
|------------|------------------------|-----------|-------------------|
| Estrutura de aço | Corrosão | Galvanização a quente (85 μm+) | 80–100 anos (não costeiro) |
